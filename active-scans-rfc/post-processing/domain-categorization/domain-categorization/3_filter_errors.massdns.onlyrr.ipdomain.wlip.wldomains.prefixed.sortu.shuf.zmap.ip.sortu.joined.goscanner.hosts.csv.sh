...
OUTFILEPREFIX=$1_$2.domain.$SORT.csv.massdns.onlyrr.ipdomain.wlip.wldomains.prefixed.sortu.shuf.zmap.ip.sortu.joined.goscanner.hosts.errors
awk -F, '$10 != "SUCCESS" {print $4}' $INFILE | sort -u -R -S100G --random-source=shuf_random_seed | head -n $3 > $BASEDIR/$OUTFILEPREFIX.$3.csv
