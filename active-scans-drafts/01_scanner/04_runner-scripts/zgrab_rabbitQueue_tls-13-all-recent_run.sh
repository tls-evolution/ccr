...
cat example_in_minimal.txt | ${ZGRAB} -http-user-agent "${USERAGENT}" --tls --http-max-redirects 20 -http-method GET -http-max-size 0 -port 443 --http="/" -tls-version=TLSv1.3 --blacklist-ip=${BLACKLIST_IP} --blacklist-dom=${BLACKLIST_DOM} -senders 3000 > example_out.json
